...
function NavLink({ url, text }: NavLink) {
  ...
  return (
    <li className="flex">
      <Link
        href={url}
        className={`flex items-center mx-4  text-[#032d60]   ${
          path.includes(url) ? "border-b-4 border-blue-400 " : ""
        }}`}
      >
        {text}
      </Link>
...
export default function Navbar({
  ...
  const handleScroll = () => {
    const scrollDistance = window.innerWidth <= 768 ? 20 : 100; // Adjust distances for mobile and desktop
  ...
  return (
    <div
      className={` mx-auto py-5 lg:py-8 px-4 lg:px-0 sticky top-0  z-50 ${
        headerBg
    ...
    >
      <div className="flex  items-center justify-between max-w-6xl mx-auto">
        ...
        <div className="hidden lg:block ms-auto p-2">
          <Link
        ...
        <Dialog.Panel className="fixed inset-y-0 rtl:left-0 ltr:right-0 z-50 w-full overflow-y-auto bg-gray-800 px-6 py-6 sm:max-w-sm sm:ring-1 sm:ring-inset sm:ring-white/10">
          <div className="flex items-center justify-between">
            <a href="#" className="-m-1.5 p-1.5">
              <span className="sr-only">Qualitia</span>
